...
const vec4 TRI_VERTS[NUM_VERTICES_PER_PARTICLE] = vec4[NUM_VERTICES_PER_PARTICLE](
    // TODO [Task 17] Calculate the triangle point offsets (see diagram in handout)
     vec4(0, 2, 0, 0),
     vec4(0, 0, 0, 0),
     vec4(2, 0, 0, 0)
);
...
void main() {
    int particleID = int(floor(gl_VertexID / 3.0));
    int triID = gl_VertexID % 3;
    // TODO [Task 17] Which particle and triangle vertex are we dealing with?
    ...
    vec4 velAge = vec4(0);
    // TODO [Task 17] sample pos and vel textures
